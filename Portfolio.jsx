...
const TECH_TAGS = ["PHP", "Laravel", "MySQL", "RESTful API", "JavaScript", "HTML/CSS", "Git", "Adobe Photoshop"];
...
function HomeSection() {
  const typed1 = useTyped(["Mahasiswa UNIBI", "Pengembang Web", "Pemrogram Kreatif"], 100, 70, 1500);
  ...
  return (
    <section
      ...
      style={{
        background: "url('https://images.unsplash.com/photo-1555066931-4365d14431b9?w=1920&q=80') no-repeat center/cover",
      }}
    >
      {/* Overlay */}
...
function AboutSection() {
  const typed2 = useTyped(
    ["Saya, M Tonny Heru Susanto Mahasiswa Informatika", "Bersemangat dalam Pemrograman", "Siap Membangun Teknologi Masa Depan"],
    90, 60, 2000
  );
  ...
  return (
    <section id="about" className="min-h-screen flex flex-col justify-center py-20 bg-white font-['Poppins',sans-serif]">
      <div ref={ref} className="max-w-[1300px] mx-auto px-5 sm:px-20">
        <SectionTitle light title="Tentang Saya" sub="Siapa saya" />
        ...
        <div className={`flex flex-col md:flex-row items-center gap-12 transition-all duration-700 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
          ...
          <div className="flex-shrink-0 flex justify-center" style={{ perspective: "1000px" }}>
            <img
              ref={imgRef}
              src="https://placehold.co/300x300/111827/ffffff?text=Tonny"
  ...
  return (
    <section id="skills" className="py-24 bg-[#111] text-white font-['Poppins',sans-serif] relative overflow-hidden">
      ...
      <div ref={ref} className="relative z-10 max-w-[1300px] mx-auto px-5 sm:px-20">
        ...
        <p className="text-center text-white/30 text-sm mb-12 tracking-widest uppercase">Teknologi yang saya kuasai</p>
...
function FeaturedSection() {
  ...
  return (
    <section id="featured" className="py-20 bg-white font-['Poppins',sans-serif]">
      <div ref={ref} className="max-w-[1300px] mx-auto px-5 sm:px-20">
        <SectionTitle light title="Featured Project" sub="Proyek Unggulan" />
...
function ContactSection() {
  ...
  const sendChat = () => {
    ...
    setMessages((m) => [...m, { id: Date.now(), sender, initial: sender[0].toUpperCase(), color, text, time, self: true }]);
    setChatInput("");
              ...
              {messages.map((m) => (
                <div key={m.id} className={`flex items-start gap-2.5 ${m.self ? "flex-row-reverse" : ""}`}>
                  ...
                  <div className={`px-3 py-2.5 max-w-[85%] rounded-[10px] ${m.self ? "bg-crimson rounded-tr-none" : "bg-[#252836] rounded-tl-none"}`}>
                    ...
                    <p className={`text-[10px] mt-1 ${m.self ? "text-right text-white/60" : "text-[#4b5563]"}`}>{m.time}</p>
                  </div>
                </div>
              ))}
              <div ref={chatEndRef} />
